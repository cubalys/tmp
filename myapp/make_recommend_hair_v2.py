...
def jsonTofacevector(face_json):
    ...
    with open("/work/urs-server/face_factors_v2.json") as json_file:
    ...
    for items in face_factors:
        for factors in face_factors[items]:
            if factors != "weight":
                weight = face_factors[items]["weight"]
                for i in range(0, face_factors[items][factors]) :
                    result = np.append(result, float(landmark[items][factors + str(i)]["y"] - reset_y)/reset_h * weight)
                    result = np.append(result, float(landmark[items][factors + str(i)]["x"] - reset_x)/reset_w * weight)
    return result
